...
const Personalization = ({ onSave }: { onSave: () => void }) => {
    ...
    const fetchCharacters = async () => {
      try {
        ...
        if (characterDetails) {
          ...
          const newFormData: Record<string, { name: string; birthYear: string; gender: string }> = {};
          ...
          Object.entries(characterDetails).forEach(([key, value]) => {
            ...
            if (key.startsWith('custom_')) {
              newCustoms.push({ key, label: relationshipWithMe || 'Custom Character' });
            }
          });
  ...
  return (
    <div className='p-6 bg-white rounded-lg shadow-md max-w-xl mx-auto'>
      ...
      <p className='mb-4 text-gray-600'>Enter names and birth years for each character:</p>
        ...
        {allCharacters.map(({ key, label }) => {
          ...
          return (
            <div key={key} className='grid grid-cols-4 gap-4 items-center'>
              ...
              {formData[key]?.birthYear && <p className='text-sm text-gray-500 col-span-4'>Age: {age} years</p>}
            </div>
          );
        })}
      </div>
      ...
      <div className='mt-6 flex gap-2 items-center'>
        ...
        />
        <button onClick={handleAddCharacter} className='bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-500'>
          Add
        </button>
      </div>

      <button onClick={handleSubmit} className='mt-6 w-full bg-green-600 text-white py-2 rounded hover:bg-green-500'>
        Save Details
      </button>
    </div>
  );
